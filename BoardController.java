...
public class BoardController extends HttpServlet {
	...
	private void list(HttpServletRequest request, HttpServletResponse response)
		...
		String psStr = request.getParameter("ps");//get방식으로 넘어온 cp와 ps를 받아옴.
		...
		if(cpStr == null) {// 따로 파라미터로 입력 들어온 값이 없으면,
			...
			if(cpObj != null) cp=(Integer)cpObj;// 그리고 cpObj에 값이 있으면, int로 형변환해서 cp에 대입함.
	...
	private void update(HttpServletRequest request, HttpServletResponse response)
		...
		if(seqStr != null) {
			...
			try {
			    seq = Integer.parseInt(seqStr);
				BoardService service = BoardService.getInstance();
				ArrayList<Board> update = service.contentS(seq);
